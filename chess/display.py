...
class ChessGUI:
        ...
        for rank in range(8):
            for file in range(8):
                ...
                self.canvas.create_text(
                    x0 + 10, y1 - 10, text=square_label, fill="black"
                )
    ...
    def on_piece_clicked(self, event):
        ...
        if piece != Piece.NONE and Piece.get_color(piece) == self.board.current_turn:
            ...
            self.selected_piece_image = self.canvas.create_image(
                event.x, event.y, image=self.piece_images[piece]
            )
        else:
    ...
    def on_piece_dropped(self, event):
        if (
            self.selected_piece_image is not None
            and self.board.selected_piece is not None
        ):
            selected_index, piece = self.board.selected_piece

            current_square = Coordinates(selected_index % 8, selected_index // 8)
            destination_square = Coordinates(
                event.x // self.square_size, 7 - event.y // self.square_size
            )

            default_move = Move(piece, current_square, destination_square)

            legal_moves = self.board.get_all_possible_moves(
    ...
    def show_promotion_options(self, color):
        ...
        while chosen_piece is None:
            ...
            buttons = {
                Piece.QUEEN: tk.Button(
                    button_frame,
                    text="Queen",
                    command=lambda: on_promotion_choice(Piece.QUEEN),
                ),
                Piece.ROOK: tk.Button(
                    button_frame,
                    text="Rook",
                    command=lambda: on_promotion_choice(Piece.ROOK),
                ),
                Piece.BISHOP: tk.Button(
                    button_frame,
                    text="Bishop",
                    command=lambda: on_promotion_choice(Piece.BISHOP),
                ),
                Piece.KNIGHT: tk.Button(
                    button_frame,
                    text="Knight",
                    command=lambda: on_promotion_choice(Piece.KNIGHT),
                ),
            }
