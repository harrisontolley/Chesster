...
class Move:
    def __init__(
        ...
        if self.get_can_en_passant():
            self.en_passant_idx = Coordinates(
                self.destination_coords.get_file(),
                self.current_coords.get_rank() + 1
                if Piece.get_color(piece) == Piece.WHITE
                else self.current_coords.get_rank() - 1,
            ).get_board_index()

        if self.get_is_en_passant():
            self.taken_piece_idx = Coordinates(
                self.destination_coords.get_file(), self.current_coords.get_rank()
            ).get_board_index()
    ...
    def __eq__(self, other) -> bool:
        return (
            ...
            and self.destination_coords == other.destination_coords
        )
