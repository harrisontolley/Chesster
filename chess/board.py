...
class Board:
    ...
    def switch_turn(self):
        self.current_turn = (
            Piece.BLACK if self.current_turn == Piece.WHITE else Piece.WHITE
        )
    ...
    def get_pawn_moves(self, piece, current_coords: Coordinates):
        ...
        # Forward move
        pushed_coords = Coordinates(
            current_coords.get_file(), current_coords.get_rank() + direction
        )
        if 0 <= pushed_coords.get_rank() < 8:
            if self.Square[pushed_coords.get_board_index()] == Piece.NONE:
                ...
                if pushed_coords.get_rank() == promotion_rank:
                    promotion_move = Move(
                        piece, current_coords, pushed_coords, is_promotion=True
                    )
                ...
                if current_coords.get_rank() == starting_rank:
                    double_pushed_coords = Coordinates(
                        current_coords.get_file(),
                        current_coords.get_rank() + 2 * direction,
                    )

                    if (
                        self.Square[double_pushed_coords.get_board_index()]
                        == Piece.NONE
                    ):
        ...
        # Captures
        for offset in [-1, 1]:
            capture_coords = Coordinates(
                current_coords.get_file() + offset,
                current_coords.get_rank() + direction,
            )
            ...
            if 0 <= capture_file < 8 and 0 <= capture_rank < 8:
                ...
                if (
                    self.Square[capture_index] != Piece.NONE
                    and Piece.get_color(self.Square[capture_index]) != color
                ):
                    promotion_rank = 7 if color == Piece.WHITE else 0
                    capture_move = Move(piece, current_coords, capture_coords)

                    if capture_rank == promotion_rank:
                        capture_promotion = Move(
                            piece, current_coords, capture_coords, is_promotion=True
                        )
        ...
        for y_offset in range(-2, 3):
            for x_offset in range(-2, 3):
                if abs(x_offset) + abs(y_offset) == 3:
                    if current_rank + y_offset in range(
                        8
                    ) and current_file + x_offset in range(8):
                        destination_coords = Coordinates(
                            current_file + x_offset, current_rank + y_offset
                        )

                        if (
                            self.Square[destination_coords.get_board_index()]
                            == Piece.NONE
                            or Piece.get_color(
                                self.Square[destination_coords.get_board_index()]
                            )
                            ...
                            knight_move = Move(
                                piece, current_coords, destination_coords
                            )
    ...
    def get_bishop_moves(self, piece, current_coords):
        ...
        for NE_offset in range(1, 8):
            if current_rank + NE_offset in range(
                8
            ) and current_file + NE_offset in range(8):
                destination_coords = Coordinates(
                    current_file + NE_offset, current_rank + NE_offset
                )
                ...
                if self.Square[destination_coords.get_board_index()] == Piece.NONE:
                    ...
                    Piece.get_color(self.Square[destination_coords.get_board_index()])
                    != color
                ):
                    bishop_move = Move(piece, current_coords, destination_coords)
                    moves[bishop_move] = bishop_move
        ...
        for SE_offset in range(1, 8):
            if current_rank - SE_offset in range(
                8
            ) and current_file + SE_offset in range(8):
                destination_coords = Coordinates(
                    current_file + SE_offset, current_rank - SE_offset
                )
        ...
        for NW_offset in range(1, 8):
            if current_rank + NW_offset in range(
                8
            ) and current_file - NW_offset in range(8):
                destination_coords = Coordinates(
                    current_file - NW_offset, current_rank + NW_offset
                )
        ...
        for SW_offset in range(1, 8):
            if current_rank - SW_offset in range(
                8
            ) and current_file - SW_offset in range(8):
                destination_coords = Coordinates(
                    current_file - SW_offset, current_rank - SW_offset
                )
    ...
    def get_king_moves(self, piece, current_coords: Coordinates):
        ...
        for x_offset in range(-1, 2):
            for y_offset in range(-1, 2):
                if current_rank + y_offset in range(
                    ...
                    if (
                        self.Square[destination_coords.get_board_index()] == Piece.NONE
                        or Piece.get_color(
                            self.Square[destination_coords.get_board_index()]
                        )
                        != color
        ...
        if self.can_castle_king_side(color):
            if (
                self.Square[current_coords.get_board_index() + 1] == Piece.NONE
                and self.Square[current_coords.get_board_index() + 2] == Piece.NONE
            ):
                king_move = Move(
                    piece, current_coords, Coordinates(current_file + 2, current_rank)
                )
                king_side_castle = Move(
                    piece,
                    current_coords,
                    Coordinates(current_file + 2, current_rank),
                    is_castling=True,
                )
        ...
        if self.can_castle_queen_side(color):
            if (
                self.Square[current_coords.get_board_index() - 1] == Piece.NONE
                and self.Square[current_coords.get_board_index() - 2] == Piece.NONE
                and self.Square[current_coords.get_board_index() - 3] == Piece.NONE
            ):
                king_move = Move(
                    piece, current_coords, Coordinates(current_file - 2, current_rank)
                )
    ...
    def promote_piece(self, move: Move):
        if move.get_is_promotion():
            self.Square[
                move.get_destination_coordinates().get_board_index()
